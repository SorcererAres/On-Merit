...
{{
    ...
    "key_strengths": ["1-5 条核心优势。每条：① **简短标题**（4-8 字，加粗）、② 引用简历里的具体公司/项目/数据、③ 说明为什么这是优势。示例："**量化成果突出**：在"XX 公司"经历中保留并强化了满意度提升 56%、投诉率下降 80% 等关键数据，使成果更具说服力。""],
    "areas_for_improvement": ["1-5 条改进建议，按优先级从高到低排列。每条：① **简短标题**（4-8 字，加粗）、② 引用简历里的具体内容指出问题、③ 给出改写方向 + 示例（用「如」举例，X% 代替具体数字）。示例："**补教育背景专业信息**：当前教育经历仅含学校名称，建议补充具体专业（如视觉传达、交互设计），这是招聘方评估专业匹配度的基础信息。"{advice_schema}
}}
