...
**色阶档位语义**（源自 Geist 设计系统，每档编码「意图」而非仅明暗；组件按此选档，可追溯）：

| 档 | 用途 | | 档 | 用途 |
|----|------|--|----|------|
| 100 | 默认背景 | | 600 | 激活边框 |
| 200 | **hover 背景** | | 700 | 实色填充（高对比） |
| 300 | **激活背景** | | 800 | 实色填充 hover |
| 400 | 默认边框 | | 900 | 次要文本/图标 |
| 500 | hover 边框 | | 1000 | 主要文本/图标 |

> 由此：`--accent = gray-200`（hover 表面）、边框用 400+、可访问文本用 900/1000。彩色 Badge/Alert 文本须用 900（700 是填充档，12px 文本对比度不足）。solid 彩色按钮 hover 不用 opacity（会连文字淡化跌破 AA），改用向黑混色的 `--destructive-hover` 类 token。

> **已知取舍（WCAG 1.4.11）**：`--border`/`--input` 用 `gray-alpha-400` 半透明淡边框，非文本对比度约 1.2:1，不满足 1.4.11 的 3:1。这是 **Geist 极简边框美学的固有特征**，本项目**有意保留**（视觉忠于 Geist）。可访问性靠：可聚焦元素的两层聚焦环、填充/背景对比、文字标签共同保证，不单靠边框。若未来要合规，只需把 `--input` 提到 ≥3:1 的实色灰（如 gray-800），保留 `--border` 淡边框即可。
