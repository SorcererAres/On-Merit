...
### 4.8 字段级 AI 润色(新端点)
- `POST /api/polish-field { text, kind: work|project|edu|summary|skills, jd? }` → `{ md, new_terms: string[] }`。
...
- **template(无 source_text)**:按 kind 返回**纯骨架**(占位符不含任何具体事实):
  work/project=STAR(背景/任务/行动/成果);education=主修课程/成绩排名/论文/奖项;
...
- **E2 表单重构**:分节手风琴 + 条目 CRUD(稳定 id)+ validateResumeForm(失焦红框 +
  日期顺序 + 诊断/下载前黄条)+ 年月/至今/回退控件 + 标签输入(描述字段暂用 Textarea)。
...
- **E5 AI 生成**:/api/generate-field(extract 双门槛 / template 四类骨架)。
...
<!-- codex-peer-reviewed: 2026-07-03T05:28:40Z rounds=4 verdict=approved -->
