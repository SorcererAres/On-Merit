...
- **联动高亮（修 codex #3，降级为「近似定位」）**：点右侧某段 → 在左侧原文按**归一化后**的最长公共子串定位并滚动高亮，规则：**最小长度阈值**（如 ≥8 字）、**命中唯一**才高亮、**歧义/多命中/无文本层则不高亮**；UI 明确标「近似定位」。`source_text` 为空（无文本层且 OCR 失败）→ 左栏只读降级、无联动。**不做 PDF 坐标级 bbox（列 v2）。**
...
3. **W2 节点1**：上传骨架屏 + 左原文/右表单核对（近似定位）+ 诊断报告（动态雷达 + 覆盖指数，诚实无提升）。依赖 W1 的 source_text。
...
- 原件 = 文本层，非像素 PDF；联动是近似定位；模板 v1 限 3–4 套——均如实标注，不过度承诺。
