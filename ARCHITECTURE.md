...
## 〇、部署形态决策：**方案 B（海外免费邀请制）**

为避开 ICP 备案与企业经营许可（个人无法做经营性服务），试点期采用**香港部署 + 免费 + 邀请制**：

- **躲掉**：ICP 备案、等保定级、网络日志留存（境内服务器义务）。
- **换来**：**个人信息出境**合规——简历是敏感 PII，境内用户数据放境外即"出境"，需**单独同意 + 出境机制
  （标准合同/认证/评估，需法律意见）+ 可能境内代表**。PIPL 有**域外效力**，海外不等于免责。
- **由此连锁调整**：
  - 部署区域 = **香港 region**（内地可直连、低延迟、无需备案）。
  - LLM **翻转为海外模型（Claude/Gemini）**：与服务器同处境外，避免"境外→境内 LLM→境外"数据来回；中文质量足够。
  - **删除整个支付模块**（免费）→ 顺带砍掉"商户进件"长板；验证信号相应变弱（测意愿/使用/投递，非真实付费）。
  - 鉴权 = **邀请码 + 邮箱 magic link**（不依赖中国短信，全链路留境外）。
- **不变**：引擎、队列租约幂等、运行隔离、沙箱、反造假、**删除闭环（PIPL 仍要）**、前端、字段纠错+逐条 diff。
- **数据最小化为核心原则**：引擎已不把姓名/院校送 LLM；再加"尽量少存、短保留、能不落地就不落地"。

> 验证为真后转**方案 C**（成立公司 + 企业备案 + 经营许可 + 收费），那时备案/进件只是清单里的普通工时。
> 以下章节为通用机制；与方案 B 冲突处以本节为准（无备案/等保、无支付、海外 LLM、邮箱登录、加出境合规）。
