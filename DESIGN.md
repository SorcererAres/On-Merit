...
| **P7** | 待做（投机性，需 LLM） | LLM 派生 Kami 叙事 section、metrics label 质量、JSON Schema 入口校验、按字段 Counter 数字校验、列表稳定 ID 匹配 diff |
...
└── test_patcher.py        # P6 离线测试（7 项，含「结构造假物理不可能」）
```

测试总计 **48 项**全过（improver 6 / resume_agent 5 / resume_diff 8 / p3b 8 / p4 9 / patcher 12）。
