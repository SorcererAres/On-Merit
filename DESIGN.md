...
| **INGEST 内置** | ✅ 已完成 | `ingest.py`：PDF -> 文本（PyMuPDF）-> LLM 结构化 -> 校验，补全闭环入口。真机：张良鹏真实 PDF 全自动抽出姓名/邮箱/5 段工作/4 技能/3 证书，首条成果忠实保留，直接渲染通过。**至此 PDF -> JSON -> 评分 -> 改写 -> 渲染 全链在项目内闭环**（91 项测试通过） |
...
├── ingest.py              # INGEST：PDF -> JSON Resume（PyMuPDF + LLM 结构化 + 校验）
